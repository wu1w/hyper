...
use hyper_loop::cron;
pub use hyper_loop::cron::CronJob;
...
impl CronStore {
    ...
    /// UI / hub jobs also land in workspace `.grok-hyper/cron.json` so ingest cannot
...
mod tests {
    ...
    fn ingest_workspace_cron_json() {
        let dir = std::env::temp_dir().join(format!("hyper-web-cron-{}", now_s()));
        fs::create_dir_all(dir.join(".grok-hyper")).unwrap();
        fs::write(
            dir.join(".grok-hyper/cron.json"),
    ...
    fn workspace_file_can_hold_ui_jobs() {
        let dir = std::env::temp_dir().join(format!("hyper-web-cron-ws-{}", now_s()));
        fs::create_dir_all(dir.join(".grok-hyper")).unwrap();
        ...
        cron::save_jobs(dir.join(".grok-hyper/cron.json"), &jobs).unwrap();
        let loaded = cron::load_jobs(dir.join(".grok-hyper/cron.json"));
