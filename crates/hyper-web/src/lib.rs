...
pub use crate::office::{docs_ready, persist_office_secret};
pub use crate::office_runtime::{
    container_running, docker_cli_ok, ensure_office, office_auto_enabled, stop_office, EnsureError,
    EnsureOpts, EnsureReport, CONTAINER, IMAGE,
};
...
pub async fn run(opts: WebOpts) -> Result<std::process::ExitCode> {
    ...
    if !ignored.is_empty() {
        eprintln!(
            "hyper web: unused HYPER_* (BASE_URL / API_KEY / MODEL / WORKING_WINDOW apply): {}",
            ignored.join(", ")
        );
    }
    ...
    let open = hyper_loop::sidecar::RpcRequest {
        jsonrpc: "2.0".into(),
    ...
    match session.handle(&open) {
        hyper_loop::sidecar::Dispatch::Result { .. } => {}
    ...
    let bridge_bind = cfg.office.bridge_bind.clone();
    let state = AppState::new(session, cfg, cfg_path, opts.agents_md, opts.agents_md_head)?;
    state.spawn_background();
    state.spawn_office_boot();
    let bridge_state = state.clone();
    tokio::spawn(async move {
        crate::office::spawn_bridge(bridge_state, &bridge_bind).await;
    });
...
fn console_dir() -> PathBuf {
    if let Ok(p) = std::env::var("HYPER_CONSOLE_DIR") {
...
/// Packaged layout: `Resources/bin/hyper` + `Resources/console/`, or the two
...
mod tests {
    ...
    fn packaged_console_sits_beside_or_above_the_binary() {
        let root = std::env::temp_dir().join(format!("hyper-pack-{}", std::process::id()));
