...
/// Imagine REST default when `[server].image_model` is empty.
...
/// Runtime-only: `HYPER_WORKING_WINDOW` replaced the on-disk value.
...
/// Apply a `HYPER_WORKING_WINDOW` value on top of the file's window.
...
pub struct Config {
    ...
    /// Last folder chosen in the console Files page. `hyper web` uses this
    /// when `--workspace` is omitted.
    #[serde(default, skip_serializing_if = "ConsoleConfig::is_unset")]
    pub console: ConsoleConfig,
    /// Self-hosted OnlyOffice Docs. Empty jwt_secret is filled on first `hyper web`.
    #[serde(default, skip_serializing_if = "OfficeConfig::is_unset")]
...
pub struct ServerConfig {
    ...
    pub api_key: String,
    pub model: String,
    /// Imagine REST origin (`POST {url}/images/generations`). Empty = same as `base_url`.
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub image_base_url: String,
    /// Imagine REST `model`. Empty = [`DEFAULT_IMAGE_MODEL`]. Independent of chat `model`.
    #[serde(default, skip_serializing_if = "String::is_empty")]
...
impl ServerConfig {
    /// Imagine origin. Empty `image_base_url` follows the chat `base_url`.
    pub fn resolved_image_base_url(&self) -> String {
        let own = self.image_base_url.trim().trim_end_matches('/');
        if !own.is_empty() {
    ...
    /// Imagine REST `model`. Empty `image_model` uses [`DEFAULT_IMAGE_MODEL`], never the chat model.
    pub fn resolved_image_model(&self) -> String {
        let own = self.image_model.trim();
        if !own.is_empty() {
            return own.to_string();
        }
...
impl PolicyConfig {
    pub fn think_budget(&self) -> crate::policy::ThinkBudget {
        self.think_budget_for(crate::family::Family::Grok46)
    }
...
impl Config {
    pub fn think_budget(&self) -> crate::policy::ThinkBudget {
        self.policy.think_budget_for(self.server.family)
    }
}
...
/// Local OnlyOffice Document Server. The console embeds DocsAPI; this crate
/// only stores how to reach the container.
#[derive(Clone, Debug, Serialize, Deserialize)]
#[serde(default)]
pub struct OfficeConfig {
    /// Browser-facing Docs origin (`api.js`). Default `http://127.0.0.1:8080`.
    pub docs_url: String,
    /// How the Document Server fetches files from hyper (Docker Desktop:
    /// `http://host.docker.internal:3850`).
    pub reach_url: String,
    /// Shared with the container `JWT_SECRET`. Generated if empty.
    pub jwt_secret: String,
    /// Tokenized file/callback listener. Default `0.0.0.0:3850`.
...
impl Default for OfficeConfig {
    fn default() -> Self {
        Self {
            docs_url: "http://127.0.0.1:8080".into(),
            reach_url: "http://host.docker.internal:3850".into(),
            jwt_secret: String::new(),
            bridge_bind: "0.0.0.0:3850".into(),
        }
    }
}
...
impl OfficeConfig {
    pub fn is_unset(&self) -> bool {
        self.jwt_secret.trim().is_empty()
            && self.docs_url == Self::default().docs_url
            && self.reach_url == Self::default().reach_url
            && self.bridge_bind == Self::default().bridge_bind
    }

    pub fn docs_origin(&self) -> String {
        self.docs_url.trim().trim_end_matches('/').to_string()
    }

    pub fn reach_origin(&self) -> String {
        self.reach_url.trim().trim_end_matches('/').to_string()
    }
    ...
    /// Fill an empty secret. Returns true when the value changed.
    pub fn fill_secret(&mut self) -> bool {
...
pub struct MediaConfig {
    pub enabled: bool,
    pub max_bytes: u64,
    /// Absolute path to `ffmpeg`, or a directory containing it. Empty = PATH / `HYPER_FFMPEG`.
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub ffmpeg: String,
    /// Absolute path to `whisper-cli`, or a directory containing it. Empty = PATH / `HYPER_WHISPER`.
    #[serde(default, skip_serializing_if = "String::is_empty")]
    pub whisper: String,
    /// ggml Whisper weights (`.bin`). Empty = `HYPER_WHISPER_MODEL` or `~/.grok-hyper/whisper/ggml-tiny.bin`.
    #[serde(default, skip_serializing_if = "String::is_empty")]
...
impl Config {
    ...
    /// Load `~/.grok-hyper/config.toml`, creating it from defaults if missing.
    ...
    pub fn load_or_init_file() -> Result<(Self, PathBuf)> {
        ...
        if !agent_md.exists() {
            ...
            crate::prompt::migrate_stale_home_agent_md(&agent_md, cfg.prompt.coding);
        }
        Ok((cfg, path))
    }

    /// Load `~/.grok-hyper/config.toml`, creating it from defaults if missing.
    /// `HYPER_BASE_URL` / `HYPER_API_KEY` / `HYPER_MODEL` overlay the file (never commit those).
    ...
    pub fn apply_env(&mut self) {
        if let Ok(u) = std::env::var("HYPER_BASE_URL") {
            let u = u.trim().to_string();
            if !u.is_empty() {
                self.server.base_url = u;
            }
        }
        if let Ok(k) = std::env::var("HYPER_API_KEY") {
            let k = k.trim().to_string();
            if !k.is_empty() {
                self.server.api_key = k;
            }
        }
        let cur = self.server.api_key.trim();
        if cur.is_empty() || cur.eq_ignore_ascii_case("local") {
        ...
        if let Ok(m) = std::env::var("HYPER_MODEL") {
            let m = m.trim().to_string();
            if !m.is_empty() {
                self.server.model = m;
            }
        }
        overlay_nonempty("HYPER_FFMPEG", &mut self.media.ffmpeg);
        overlay_nonempty("HYPER_WHISPER", &mut self.media.whisper);
        overlay_nonempty("HYPER_WHISPER_MODEL", &mut self.media.whisper_model);
        let env_w = std::env::var("HYPER_WORKING_WINDOW").ok();
        let (window, overlay) =
    ...
    pub fn env_overlay_set() -> serde_json::Value {
        serde_json::json!({
            "base_url": env_nonempty("HYPER_BASE_URL"),
            "api_key": env_nonempty("HYPER_API_KEY"),
            "model": env_nonempty("HYPER_MODEL"),
            "working_window": env_nonempty("HYPER_WORKING_WINDOW"),
        })
    }

    /// Patch the on-disk file. Never starts from an env-overlaid clone, so
    /// `HYPER_*` cannot replace `[server]` / `working_window` on save.
...
mod tests {
    ...
    fn coding_baseline_is_262k() {
        ...
        assert_eq!(c.context.working_window, 500_000);
        assert_eq!(c.context.hard_cap, 500_000);
    ...
    fn overlay_is_not_written_to_toml() {
        ...
        assert!(
            !toml.contains("[office]"),
            "empty office jwt_secret should stay off disk: {toml}"
        );
    }

    #[test]
    fn old_toml_without_office_section_loads() {
        let raw = Config::default().to_toml();
        assert!(!raw.contains("[office]"), "{raw}");
        let cfg: Config = toml::from_str(&raw).unwrap();
        assert!(cfg.office.jwt_secret.is_empty());
        assert_eq!(cfg.office.docs_origin(), "http://127.0.0.1:8080");
        let mut filled = cfg;
        assert!(filled.office.fill_secret());
        assert!(!filled.office.fill_secret());
        assert_eq!(filled.office.jwt_secret.len(), 64);
    }

    #[test]
    fn user_home_is_set_on_this_host() {
        assert!(user_home().is_some());
        assert!(Config::home_dir().unwrap().ends_with(".grok-hyper"));
    }
    ...
    #[test]
    fn image_base_url_inherits_chat_origin() {
        let mut cfg = ServerConfig::default();
        cfg.base_url = "https://api.x.ai/v1".into();
        assert_eq!(cfg.resolved_image_base_url(), "https://api.x.ai/v1");
        cfg.image_base_url = "https://imagine.example/v1/".into();
        assert_eq!(cfg.resolved_image_base_url(), "https://imagine.example/v1");
    }

    #[test]
    fn save_to_roundtrip_image_base_url() {
        let dir = std::env::temp_dir().join(format!("hyper-cfg-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("config.toml");
        let mut cfg = Config::default();
        ...
        cfg.save_to(&path).unwrap();
        let loaded = Config::load_from(&path).unwrap();
        assert_eq!(loaded.server.image_base_url, "https://imagine.example/v1");
        let raw = std::fs::read_to_string(&path).unwrap();
        assert!(raw.contains("image_base_url"), "{raw}");
        let _ = std::fs::remove_dir_all(dir);
    }

    #[test]
    fn image_model_does_not_follow_chat_model() {
        let mut cfg = ServerConfig::default();
        cfg.model = "grok-4.6".into();
        assert_eq!(cfg.resolved_image_model(), DEFAULT_IMAGE_MODEL);
        cfg.image_model = "flux-schnell".into();
        assert_eq!(cfg.resolved_image_model(), "flux-schnell");
    }

    #[test]
    fn save_to_roundtrip_image_model() {
        let dir = std::env::temp_dir().join(format!("hyper-cfg-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("config.toml");
        let mut cfg = Config::default();
        cfg.server.image_model = "grok-imagine-image-2.0".into();
        cfg.save_to(&path).unwrap();
        let loaded = Config::load_from(&path).unwrap();
        assert_eq!(loaded.server.image_model, "grok-imagine-image-2.0");
        let raw = std::fs::read_to_string(&path).unwrap();
        assert!(raw.contains("image_model"), "{raw}");
        let _ = std::fs::remove_dir_all(dir);
    }

    #[test]
    fn save_to_roundtrip_262k() {
        let dir = std::env::temp_dir().join(format!("hyper-cfg-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(&dir).unwrap();
        let path = dir.join("config.toml");
        let mut cfg = Config::default();
        cfg.context.working_window = CODING_CTX_TOKENS;
        cfg.context.hard_cap = CODING_CTX_TOKENS;
        cfg.save_to(&path).unwrap();
        let loaded = Config::load_from(&path).unwrap();
        assert_eq!(loaded.context.working_window, CODING_CTX_TOKENS);
        assert_eq!(loaded.context.hard_cap, CODING_CTX_TOKENS);
        assert_eq!(
            parse_working_window(CODING_CTX_TOKENS).unwrap(),
            CODING_CTX_TOKENS
        );
