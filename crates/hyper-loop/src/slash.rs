...
const REJECT: &[(&str, &str)] = &[
    ("personality", "hyper has no personality overlays"),
    ("skin", "TUI theme is not in hyper yet"),
    ("voice", "voice mode is not in hyper"),
    ...
    (
        "platforms",
        "configure [[channels.endpoints]] and run hyper --channels",
    ),
    (
        "gateway",
        "configure [[channels.endpoints]] and run hyper --channels",
    ...
    ("learn", "author a SKILL.md in ~/.grok-hyper/skills instead"),
    ...
    (
        "dump_history",
        "JSONL is the dump: ~/.grok-hyper/sessions/<id>.jsonl",
    ),
...
pub fn parse_slash(text: &str) -> Option<SlashCmd> {
    ...
    match name.as_str() {
        ...
        "undo" | "rewind" => rest.is_empty().then_some(SlashCmd::Undo),
        ...
        "fast" => {
            if rest.is_empty() {
                ...
                Some(SlashCmd::Unsupported {
                    name: "/fast is thinking-off in hyper, not cloud Priority Processing".into(),
                })
            }
        }
        "think" | "effort" => parse_think(&rest),
        ...
        "title" | "rename" => parse_title_rename(&rest),
        ...
        "imagine" | "image" => parse_imagine(&rest),
        "approvals" | "approval" => parse_approvals(&rest),
        "lossy" | "low-precision" | "lowprecision" => parse_lossy(&rest),
        "yolo" | "always-approve" | "always_approve" => {
            rest.is_empty().then_some(SlashCmd::Approvals {
...
fn parse_fork(rest: &str) -> SlashCmd {
    let mut parts = Vec::new();
    for p in rest.split_whitespace() {
        if p.eq_ignore_ascii_case("--worktree") || p.eq_ignore_ascii_case("--no-worktree") {
            continue;
        }
        parts.push(p);
    }
    SlashCmd::Fork {
        directive: nonempty(parts.join(" ")),
    }
}

fn parse_title_rename(rest: &str) -> Option<SlashCmd> {
    let rest = rest.trim();
    if rest.eq_ignore_ascii_case("--auto") {
        return Some(SlashCmd::Title {
            name: String::new(),
        });
    }
    if rest.is_empty() {
...
fn parse_imagine(rest: &str) -> Option<SlashCmd> {
    match rest.trim().to_ascii_lowercase().as_str() {
        "" | "on" => Some(SlashCmd::Imagine {
            on: Some(true),
            prompt: None,
        }),
        "off" | "exit" | "quit" => Some(SlashCmd::Imagine {
            on: Some(false),
            prompt: None,
        }),
        _ => Some(SlashCmd::Imagine {
            on: Some(true),
...
fn parse_think(rest: &str) -> Option<SlashCmd> {
    ...
    match rest.to_ascii_lowercase().as_str() {
        ...
        "high" => Some(SlashCmd::Think(Effort::High)),
...
pub fn help_text() -> String {
    "\
hyper slash commands (Hermes-shaped; local unless noted)
...
Session
  ...
  /title <name>          name the session (alias /rename)
  /rename --auto         clear title (auto-title is not implemented)
  /fork [directive]      copy this session to a new id (--worktree ignored)
  ...
  /undo                  tombstone last user turn (alias /rewind)
...
Plan / permissions (TUI; --print stays YOLO)
  ...
  /approvals ask|auto|yolo
                         ...
                         yolo = never prompt (alias /yolo, /always-approve)
...
Config
  ...
  /cron [add|rm|list]    控制台定时任务（写 .grok-hyper/cron.json）
  /version /config
"
    .to_string()
}
...
pub fn mcp_text(reg: &McpRegistry) -> String {
    if reg.servers.is_empty() {
        return "**MCP**\n\nNone. Drop `[[mcp.servers]]` in config.toml or `.grok-hyper/mcp.toml`."
            .into();
    }
...
pub fn skills_text(catalog: &SkillCatalog) -> String {
    if catalog.skills.is_empty() {
        return "**Skills**\n\nNone under ~/.grok-hyper/skills. Drop a folder with SKILL.md."
            .into();
    }
...
pub fn model_text(current: &str, args: &str) -> ModelAction {
    ...
    if args.is_empty() || args.eq_ignore_ascii_case("list") || args == "-h" || args == "help" {
        return ModelAction::Show(format!(
            "**Model** {current}\n\n\
             hyper talks to one OpenAI-compat endpoint (`config.toml` / HYPER_MODEL).\n\
             `/model <name>` session-only. `/model <name> --global` writes config.toml.\n\
             `/setup` (or `/model setup`) walks base_url, api_key, model in the TUI.\n\
             HYPER_* overlays CLI/TUI/web; `/model --global` writes the file."
        ));
    }
...
pub fn version_text() -> String {
    format!(
        "hyper {}  family=qwen38  loop=hyper-loop",
        env!("CARGO_PKG_VERSION")
    )
}

pub fn setup_text() -> String {
    "**Setup** is a TUI walk (`hyper` with no prompt).\n\n\
     Fields: base_url → api_key → model. Saved to ~/.grok-hyper/config.toml.\n\
     Or edit that file and set HYPER_BASE_URL / HYPER_API_KEY / HYPER_MODEL."
        .into()
}
...
pub fn plan_text(on: bool) -> String {
    if on {
        format!(
            "**Plan mode on.** Read-only except `plan.md` (session plan file). \
             Write a markdown plan there, then `/plan go` \
        ...
        "**Plan mode off.** `/plan` to research read-only before edits (only `plan.md` is writable).".into()
    }
}
...
pub fn imagine_text(on: bool) -> String {
    if on {
        "**Imagine on.** The next prompt calls `POST {生图端点}/images/generations` \
         instead of the agent. `/imagine off` returns to chat.\n\
         `/imagine <prompt>` turns it on and generates immediately."
            .into()
    } else {
        "**Imagine off.** `/imagine` or the composer + menu to generate images via the Imagine REST endpoint.".into()
    }
}
...
mod tests {
    ...
    #[test]
    fn hermes_names_and_hyper_depth() {
        ...
        assert_eq!(
            parse_slash("/imagine"),
            Some(SlashCmd::Imagine {
                on: Some(true),
                prompt: None
            })
        );
        assert_eq!(
            parse_slash("/imagine off"),
            Some(SlashCmd::Imagine {
                on: Some(false),
                prompt: None
            })
    ...
    #[test]
    fn parse_plan_rewind_fork_aliases() {
        assert_eq!(
            parse_slash("/plan go"),
            Some(SlashCmd::Plan {
                action: PlanAction::Go,
        ...
        assert_eq!(
            parse_slash("/plan implement auth"),
            Some(SlashCmd::Plan {
                action: PlanAction::On,
                prompt: Some("implement auth".into())
            })
        );
        assert_eq!(parse_slash("/rewind"), Some(SlashCmd::Undo));
        assert_eq!(
            parse_slash("/rename x"),
            Some(SlashCmd::Title { name: "x".into() })
        );
        assert_eq!(
            parse_slash("/rename --auto"),
            Some(SlashCmd::Title {
                name: String::new()
            })
        );
        assert_eq!(
            parse_slash("/always-approve"),
            Some(SlashCmd::Approvals {
                mode: Some(ApprovalMode::Yolo)
            })
        );
        assert_eq!(
            ...
            Some(SlashCmd::Think(Effort::High))
        );
        assert_eq!(
            parse_slash("/fork"),
            Some(SlashCmd::Fork { directive: None })
        );
        assert_eq!(
            parse_slash("/fork --worktree keep going"),
            Some(SlashCmd::Fork {
                directive: Some("keep going".into())
            })
        );
        assert_eq!(
            parse_slash("/fork --no-worktree"),
            Some(SlashCmd::Fork { directive: None })
        );
        assert!(parse_slash("/fork").is_some());
        assert!(!matches!(
            parse_slash("/fork"),
            Some(SlashCmd::Unsupported { .. })
        ));
