...
pub fn vendor_root() -> PathBuf {
    if let Ok(p) = std::env::var("HYPER_VENDOR_DIR") {
        let p = p.trim();
        if !p.is_empty() {
...
/// Packaged layout: `Resources/bin/hyper` + `Resources/vendor/qwen38/…`.
...
mod tests {
    ...
    fn packaged_vendor_sits_beside_or_above_the_binary() {
        let root = std::env::temp_dir().join(format!("hyper-vendor-{}", std::process::id()));
