...
pub use compact::{
    ...
    run_official_compact_input, should_official_compact, unwrap_qwen_hidden, OfficialCompaction,
    OfficialPersist, TransportCompact, PRICE_CLIFF_TOKENS,
};
...
mod tests {
    ...
    fn tmp_dir() -> std::path::PathBuf {
        let dir = std::env::temp_dir().join(format!("hyper-sess-{}", new_session_id()));
    ...
    #[test]
    fn derive_restores_assistant_generated_media() {
        let dir = tmp_dir();
        let mut log =
            SessionLog::create_in(&dir, start("s-aimg", SessionMode::Agent, "sys")).unwrap();
        log.append(SessionEvent::assistant("here", "", None).with_media(vec![
            crate::session::StoredMedia {
                kind: "image".into(),
                mime: "image/jpeg".into(),
                url: ".grok-hyper/generated/imagine-abcd.jpg".into(),
            },
        ]))
        .unwrap();
        let msgs = derive_messages(log.events());
        ...
        assert_eq!(a.parts[0].url, ".grok-hyper/generated/imagine-abcd.jpg");
        let _ = fs::remove_dir_all(dir);
    }
