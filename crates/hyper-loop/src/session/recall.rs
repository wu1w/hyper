...
mod tests {
    ...
    fn format_event_prints_full_blob_and_media_paths() {
        let sha = "a".repeat(64);
        let ev = SessionEvent::tool_folded(
            "c1",
            "ComputerUse",
            "screenshot ok",
            Some(sha.clone()),
        ...
        .with_media(vec![StoredMedia {
            ...
            url: ".grok-hyper/generated/shot.jpg".into(),
        }]);
        let text = format_event(9, &ev);
        assert!(text.contains(&sha), "{text}");
        assert!(
            text.contains("media=image/jpeg .grok-hyper/generated/shot.jpg"),
            "{text}"
        );
    }
}
