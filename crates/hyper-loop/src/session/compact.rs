...
pub fn apply_compact(messages: &[ChatMessage], plan: &CompactEvent) -> Vec<ChatMessage> {
    ...
    if let Some(system) = messages.first() {
        ...
        if let Some(c) = sys.content.as_mut() {
            *c = crate::platform_prefix::wash_platform_injection(c);
        }
        out.push(sys);
    }
...
fn live_task(events: &[SessionEvent], keep_user: usize) -> String {
    match events.get(keep_user) {
        Some(SessionEvent::User(u)) if !is_hidden_user_text(&u.text) => {
            let washed = crate::platform_prefix::wash_message_content("user", &u.text);
            clip(&washed, CLIP * 2)
        }
        _ => String::new(),
    }
        ...
        match event {
            ...
            SessionEvent::Assistant(a) => {
                ...
                if spoken.trim().is_empty() {
                    let reasoning = crate::platform_prefix::wash_platform_injection(&a.reasoning);
                    if let Some(note) = think_note(&reasoning, a.tool_calls.as_deref()) {
    ...
    for msg in messages.iter().skip(1) {
        match msg.role.as_str() {
            ...
            "assistant" => {
                ...
                let mut ev = SessionEvent::assistant(content, reasoning, calls);
                if !msg.parts.is_empty() {
                    ev = ev.with_media(
                        msg.parts
                            .iter()
...
mod tests {
    ...
    #[test]
    fn archive_washes_hosted_identity() {
        let events = vec![
            start(),
            SessionEvent::user("edit notes.md"),
            SessionEvent::assistant(
                "You are Grok, a helpful and maximally truthful AI built by xAI.\nEdited notes.md",
                "You are Grok, built by xAI. thinking about the file",
                None,
            ),
        ...
        let idx = &plan.index;
        assert!(
            !body.contains("You are Grok") && !idx.contains("You are Grok"),
            "identity leaked into archive:\n{body}\n{idx}"
        );
        assert!(
            !body.contains("maximally truthful") && !idx.contains("maximally truthful"),
            "{body}\n{idx}"
        );
        assert!(
            body.contains("Edited notes.md")
                || idx.contains("Edited notes.md")
                || idx.contains("notes.md"),
            "office work was washed away:\n{body}\n{idx}"
        );
    }

    #[test]
    fn apply_compact_washes_system_identity() {
        let msgs = vec![
            ChatMessage::system(
        ...
        assert!(!sys.contains("You are Grok,"), "{sys}");
        assert!(sys.contains("grok-hyper"), "{sys}");
    }
    ...
    fn useful_think_becomes_a_note_restatement_does_not() {
        ...
        let noisy = vec![
            ...
            SessionEvent::tool("b", "read", "     1|hello-from-hyper-live"),
            SessionEvent::user("next"),
        ];
        ...
        assert!(
            plan.summary.contains("hello-from-hyper-live"),
            "{}",
            plan.summary
        );
    }
