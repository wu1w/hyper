...
/// Persist the session the local console should reopen after quit / `hyper web` restart.
...
/// console. Empty leftover jsonls from older `hyper web` boots are skipped when
...
mod tests {
    ...
    fn tmp() -> PathBuf {
        let dir = std::env::temp_dir().join(format!("hyper-cat-{}", new_session_id()));
