...
mod tests {
    ...
    fn agent_prefix_under_800_tokens() {
        ...
        assert!(
            n <= 8000,
            "agent rendered prefix {n} tokens exceeds 8000 gate:\n{text}"
        );
    }
    ...
    fn coding_prompt_prefix_under_800_tokens() {
        ...
        assert!(
            n <= 8000,
            "coding prompt rendered prefix {n} tokens exceeds 8000 gate:\n{text}"
        );
    }
    ...
    fn periphery_prefix_under_800_tokens() {
        ...
        assert!(
            n <= 8000,
            "periphery rendered prefix {n} tokens exceeds 8000 gate:\n{text}"
        );
        let frozen = qwen38(
            &[
        ...
        assert!(frozen_n <= 8000, "frozen Cursor tools still {frozen_n}");
    }
