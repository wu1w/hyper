...
impl ToolCoordinator {
    ...
    pub fn take_finished(&self) -> Vec<(String, ToolResponse)> {
        lock_unpoison(&self.finished)
            .drain(..)
            .filter(|f| !super::bgwait::is_taken(&f.response.id))
        ...
        Fut: Future<Output = ToolResponse> + Send + 'static,
    {
        ...
        let now = Instant::now();
        let force_offload = shell_wants_background(&call);
        let mut deadlines = Deadlines {
            started_at: now,
            offload_at: if force_offload {
                Some(now)
            } else {
                timeout.map(|s| now + secs_to_dur(s * OFFLOAD_TIMEOUT_RATIO))
            },
            kill_at: None,
        };
        ...
        let outcome = self
            .drive(call.id.clone(), join, cancel, deadlines, force_offload)
            .await;
    ...
    fn spawn_watch(
        ...
        tokio::spawn(async move {
            ...
            super::bgwait::finish(&id, response.clone());
