...
const TASK: &str = r#"{"type":"function","function":{"name":"Task","description":"Spawn a scoped subagent (depth 1). Types: explore (read-only, low effort), plan, office, generalPurpose. Omit model to pick effort from the type. isolation none and auto share the parent cwd. isolation worktree checks out HEAD and keeps that directory after exit. Do not use for a single file read.","parameters":{"type":"object","properties":{"prompt":{"type":"string"},"description":{"type":"string","description":"Short label for the UI."},"subagent_type":{"type":"string","enum":["explore","plan","generalPurpose","office","general-purpose"]},"model":{"type":"string","description":"Optional model id. Does not change effort routing."},"background":{"type":"boolean"},"resume":{"type":"string"},"isolation":{"type":"string","enum":["none","worktree","auto"],"description":"none and auto share the parent cwd. worktree is an explicit git worktree kept after the child finishes."}},"required":["prompt"]}}}"#;
...
pub fn is_parallel_safe(name: &str) -> bool {
    matches!(
        dispatch_name(name),
        "read" | "view" | "grep" | "glob" | "search" | "web"
    )
}
