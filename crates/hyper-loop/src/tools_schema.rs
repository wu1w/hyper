...
const READ_LINTS: &str = r#"{"type":"function","function":{"name":"ReadLints","description":"Read current compiler or linter errors for code files. Call this after edits when you need diagnostics. Write and StrReplace do not attach a compiler check automatically.","parameters":{"type":"object","properties":{"paths":{"type":"array","items":{"type":"string"},"description":"Workspace-relative code files to check."},"path":{"type":"string","description":"Single-file compatibility alias."}}}}}"#;
...
const SHELL: &str = r#"{"type":"function","function":{"name":"Shell","description":"Run a command in the workspace. Optional working_directory, block_until_ms, background. Prefer Read/Grep/Glob over cat/ls/rg. Do not parallel Shell calls that touch the same path.","parameters":{"type":"object","properties":{"command":{"type":"string"},"working_directory":{"type":"string"},"block_until_ms":{"type":"integer","description":"How long to wait before backgrounding. 0 backgrounds immediately. Default is the tool timeout."},"background":{"type":"boolean","description":"Return immediately with this call id; wait with AwaitShell."}},"required":["command"]}}}"#;
...
const SWITCH_MODE: &str = r#"{"type":"function","function":{"name":"SwitchMode","description":"Switch how the current agent run operates. plan inspects and writes only plan.md; agent performs the work; ask is read-only and gathers a blocking user choice via AskQuestion before proceeding. Use this tool instead of asking the user to type a slash command.","parameters":{"type":"object","properties":{"mode":{"type":"string","enum":["agent","plan","ask"]},"target_mode_id":{"type":"string","enum":["agent","plan","ask"],"description":"Cursor alias for mode."}}}}}"#;
...
/// When `input` is false, advertise screenshot/list/wait only. Click/type
/// remain executable so a stale schema still returns a clear Error.
pub fn computer_use_tool_for(input: bool) -> Value {
    let mut v = computer_use_tool();
    if input {
        return v;
    }
    v["function"]["description"] = serde_json::json!(
        "See this Windows or macOS desktop. Screenshot works independently of click/type. Accessibility is not granted, so click, double_click, right_click, move, drag, scroll, type, and key will fail until the user enables Accessibility for grok-hyper in System Settings → Privacy & Security. Prefer screenshot | list_displays | wait. Coordinates are in the last screenshot, origin top-left. Not parallel-safe."
    );
    v["function"]["parameters"]["properties"]["action"]["description"] = serde_json::json!(
        "screenshot | list_displays | wait. click/type/drag/scroll/key need Accessibility, which is not granted."
    );
    v
}
