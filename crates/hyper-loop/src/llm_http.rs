...
//!
//! Cursor keeps the turn alive across blips: longer connect, TCP/H2 keepalive,
...
/// Painted before Agent::new / the first HTTP hop so Windows isn't silent.
...
mod tests {
    ...
    fn remote_connect_floor_covers_old_five_second_default() {
        ...
        assert_eq!(
            effective_connect_timeout_s_for(5, "https://api.x.ai/v1"),
            20
        );
        assert_eq!(
            effective_connect_timeout_s_for(45, "https://api.x.ai/v1"),
            45
        );
        assert_eq!(
