...
/// Prompt plus optional media. Queue/redirect store this; text-only steer
/// still rides [`SteerSlot`] (hidden notes cannot carry media).
...
impl Inbound {
    ...
    pub fn parts(prompt: impl Into<String>, parts: Vec<MediaPart>) -> Self {
        Self {
            prompt: prompt.into(),
            parts,
        }
...
impl Mailbox {
    ...
    pub fn offer_while_busy(&mut self, text: String) -> BusyDecision {
        self.offer_parts(text, Vec::new())
    }

    pub fn offer_parts(&mut self, text: String, parts: Vec<MediaPart>) -> BusyDecision {
        let inbound = Inbound {
            prompt: text,
            parts,
        };
        match self.busy {
            ...
            BusyPolicy::Steer => {
                if inbound.parts.is_empty() {
                    push_steer(&self.steer, inbound.prompt);
                } else {
                    // Text can still inject at the next tool boundary; media
                    // cannot ride a hidden steer note, so it queues as a turn.
                    push_steer(&self.steer, inbound.prompt.clone());
                    self.queue.push_back(inbound);
                }
    ...
    pub fn push_queue(&mut self, text: String) {
        self.push_inbound(text, Vec::new());
    }

    pub fn push_inbound(&mut self, text: String, parts: Vec<MediaPart>) {
        self.queue.push_back(Inbound {
            prompt: text,
            parts,
        });
    ...
    pub fn take_redirect(&mut self) -> Option<Inbound> {
        self.redirect.take()
    }

    pub fn pop_queue(&mut self) -> Option<Inbound> {
    ...
    pub fn peek_queue(&self) -> Vec<String> {
        self.queue.iter().map(|i| i.prompt.clone()).collect()
    }
...
mod tests {
    ...
    fn interrupt_stashes_redirect() {
        ...
        assert_eq!(
            m.take_redirect().map(|i| i.prompt).as_deref(),
            Some("fix auth")
        );
    }
    ...
    fn queue_then_pop_fifo() {
        ...
        assert_eq!(m.pop_queue().map(|i| i.prompt).as_deref(), Some("a"));
        assert_eq!(m.pop_queue().map(|i| i.prompt).as_deref(), Some("b"));
    }

    #[test]
    fn queue_keeps_media_parts() {
        let mut m = Mailbox {
            busy: BusyPolicy::Queue,
            ..Mailbox::default()
        };
        let img = crate::media::MediaPart::image_url("https://example.com/a.png");
        assert_eq!(
            m.offer_parts("see this".into(), vec![img.clone()]),
            BusyDecision::Queued
        );
        let got = m.pop_queue().expect("queued");
        assert_eq!(got.prompt, "see this");
        assert_eq!(got.parts, vec![img]);
    }

    #[test]
    fn steer_with_media_injects_text_and_queues_follow_up() {
        let mut m = Mailbox {
            busy: BusyPolicy::Steer,
            ..Mailbox::default()
        };
        let img = crate::media::MediaPart::image_url("https://example.com/b.png");
        assert_eq!(
            m.offer_parts("look".into(), vec![img.clone()]),
            BusyDecision::Steered
        );
        assert_eq!(m.peek_steer(), vec!["look".to_string()]);
        let got = m.pop_queue().expect("media follow-up");
        assert_eq!(got.prompt, "look");
        assert_eq!(got.parts, vec![img]);
    }

    #[test]
    fn text_only_steer_does_not_queue() {
        let mut m = Mailbox {
            busy: BusyPolicy::Steer,
            ..Mailbox::default()
        };
        assert_eq!(m.offer_while_busy("focus".into()), BusyDecision::Steered);
        assert_eq!(m.queued(), 0);
        assert_eq!(m.peek_steer(), vec!["focus".to_string()]);
    }
