//! Exclusive flock so `hyper web` and `hyper --channels` cannot both long-poll
...
pub fn acquire(kind: &str, id: &str) -> Result<PollLock> {
    ...
    let dir = Config::home_dir()
        ...
        .unwrap_or_else(|_| PathBuf::from("/tmp/hyper-channels"));
...
pub fn acquire_in(dir: &Path, kind: &str, id: &str) -> Result<PollLock> {
    ...
    if file.try_lock_exclusive().is_err() {
        ...
        return Err(Error::msg(format!(
            "another Hyper process is already polling {kind} `{id}`{extra}; stop `hyper web` or `hyper --channels` so they do not share a cursor"
        )));
    }
    file.set_len(0)?;
...
mod tests {
    ...
    fn webhook_skips_lock_file() {
        let dir =
            std::env::temp_dir().join(format!("hyper-lock-{}", uuid::Uuid::new_v4().simple()));
