...
pub const STUTTER_NOTE: &str = "[trajectory] Visible output is repeating in place. \
Collapse to one conclusion, or take a next step that can produce new evidence.";
...
pub const DUMP_NOTE: &str = "[trajectory] The visible answer is repeating, plus low-information scratch or cleanup. \
If the task is done, finish; otherwise name the missing evidence and take only the step that fills it.";
...
This is a different task from architecture review and names different files on purpose.";
    ...
    fn placeholder_write_is_ellipsis_path() {
        ...
        assert!(is_placeholder_path("/Users/william/grok-hyper/..."));
        assert!(is_placeholder_path("./..."));
