...
"#;
    ...
    async fn live_agent_usage_tour() {
        let cfg = live_cfg();
        let dir =
            std::env::temp_dir().join(format!("hyper-live-tour-{}", uuid::Uuid::new_v4().simple()));
        ...
        assert!(
            ...
            "mcp should mount from .grok-hyper/mcp.toml"
        );
...
Quote the project id token in the final sentence. Then stop. Do not bash.",
        ...
        while let Some(q) = mailbox.pop_queue() {
            n += 1;
            let out = agent.run(&q.prompt).await.unwrap();
        ...
        if let Some(p) = mailbox.take_redirect() {
            let out = agent.run(&p.prompt).await.unwrap();
...
twice in separate tool calls. Append only the first line of fat.txt to journal.md. Then stop.",
        ...
        let report_path = PathBuf::from("/tmp/grok-hyper-tour.json");
