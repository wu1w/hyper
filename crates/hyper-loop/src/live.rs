...
//! `HYPER_BASE_URL` / `HYPER_API_KEY` / `HYPER_MODEL` — never put tokens in git.
//!
...
mod tests {
    ...
    async fn live_agent_read_then_reply() {
        ...
        std::fs::write(dir.join("note.txt"), "hello-from-hyper-live\n").unwrap();
        ...
        opts.max_steps = 6;
        opts.home = Some(dir.join(".hyper-home"));
        ...
        let out = agent
            ...
            .unwrap();
        assert!(
            out.text.contains("hello-from-hyper-live"),
            "agent text={}",
            out.text
        );
    ...
    async fn live_agent_loop_observe() {
        ...
        // 1. Frozen-four read → answer (core ReAct).
        {
            let dir = std::env::temp_dir()
                .join(format!("hyper-obs-read-{}", uuid::Uuid::new_v4().simple()));
            std::fs::create_dir_all(&dir).unwrap();
            std::fs::write(dir.join("note.txt"), "hello-from-hyper-live\n").unwrap();
        ...
        {
            let dir = std::env::temp_dir()
                .join(format!("hyper-obs-edit-{}", uuid::Uuid::new_v4().simple()));
            std::fs::create_dir_all(&dir).unwrap();
            let completer = HttpCompleter::connect(&cfg, policy.clone()).await.unwrap();
            let mut agent = Agent::new(completer, core_opts(&cfg, dir.clone())).unwrap();
            let out = agent
        ...
        {
            let dir = std::env::temp_dir()
                .join(format!("hyper-obs-par-{}", uuid::Uuid::new_v4().simple()));
            std::fs::create_dir_all(&dir).unwrap();
        ...
        {
            let dir = std::env::temp_dir()
                .join(format!("hyper-obs-err-{}", uuid::Uuid::new_v4().simple()));
            std::fs::create_dir_all(&dir).unwrap();
        ...
        {
            let dir = std::env::temp_dir()
                .join(format!("hyper-obs-txt-{}", uuid::Uuid::new_v4().simple()));
            std::fs::create_dir_all(&dir).unwrap();
            let completer = HttpCompleter::connect(&cfg, policy.clone()).await.unwrap();
            let mut agent = Agent::new(completer, core_opts(&cfg, dir.clone())).unwrap();
            let out = agent
        ...
        {
            let dir = std::env::temp_dir()
                .join(format!("hyper-obs-nar-{}", uuid::Uuid::new_v4().simple()));
            std::fs::create_dir_all(&dir).unwrap();
    ...
    async fn live_agent_compacts_then_recalls() {
        ...
        if compacted {
            let notes = std::fs::read_dir(dir.join(".hyper-home").join("memory"))
    ...
    async fn live_watchdog_soft_nudge_on_long_think() {
        let cfg = live_cfg();
        let dir =
            std::env::temp_dir().join(format!("hyper-live-wd-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(&dir).unwrap();
        let mut policy = ThinkPolicy::agent_default();
    ...
    async fn live_doom_loop_repeated_read() {
        let cfg = live_cfg();
        let dir =
            std::env::temp_dir().join(format!("hyper-live-doom-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    async fn live_agent_memory_search_roundtrip() {
        let cfg = live_cfg();
        let dir =
            std::env::temp_dir().join(format!("hyper-live-mem-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    async fn live_agent_view_image_roundtrip() {
        let cfg = live_cfg();
        let dir =
            std::env::temp_dir().join(format!("hyper-live-view-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    async fn live_agent_view_video_stills() {
        ...
        let dir =
            std::env::temp_dir().join(format!("hyper-live-vid-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(&dir).unwrap();
        let clip = dir.join("clip.mp4");
        hyper_loop_encode_red(ffmpeg, &clip).await;
        let policy = ThinkPolicy::agent_default();
        let completer = HttpCompleter::connect(&cfg, policy).await.unwrap();
    ...
    async fn hyper_loop_encode_red(ffmpeg: &std::path::Path, clip: &std::path::Path) {
    ...
    async fn live_agent_view_audio_transcript() {
        ...
        assert!(
            ...
            "whisper ggml model missing (set HYPER_WHISPER_MODEL or put ggml-tiny.bin under the hyper home whisper dir)"
        );
        let dir =
            std::env::temp_dir().join(format!("hyper-live-aud-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(&dir).unwrap();
    ...
    fn bench_root() -> std::path::PathBuf {
        std::path::PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../hyper-bench/fixtures")
    }
    ...
    async fn live_bench_read_edit() {
        let cfg = live_cfg();
        let dir =
            std::env::temp_dir().join(format!("hyper-live-01-{}", uuid::Uuid::new_v4().simple()));
...
The file must contain println!(\"new\") and must not contain old. Change only what is required.",
    ...
    async fn live_bench_multi_file() {
        let cfg = live_cfg();
        let dir =
            std::env::temp_dir().join(format!("hyper-live-02-{}", uuid::Uuid::new_v4().simple()));
        ...
        let out = agent
            .run(
                "Read hint.txt. Change the BANNER constant in src/lib.rs so the program would print hyper-ok. \
Do not change src/main.rs.",
        ...
        assert!(
            lib.contains("hyper-ok") && !lib.contains("\"alpha\""),
            "lib.rs={lib:?} text={}",
            out.text
        );
        assert!(
            !main.contains("hyper-ok"),
    ...
    async fn live_bench_test_fix() {
        let cfg = live_cfg();
        let dir =
            std::env::temp_dir().join(format!("hyper-live-03-{}", uuid::Uuid::new_v4().simple()));
...
Do not change the test.",
    ...
    async fn live_roleplay_tools_compact_recall() {
        let cfg = live_cfg();
        let dir =
            std::env::temp_dir().join(format!("hyper-live-rp-{}", uuid::Uuid::new_v4().simple()));
        let sess = dir.join("sessions");
...
Reply in-character with one short sentence, then stop. Do not read the next day."
    ...
    async fn live_soak_tool_rounds() {
        ...
        let dir =
            std::env::temp_dir().join(format!("hyper-live-soak-{}", uuid::Uuid::new_v4().simple()));
        let sess = dir.join("sessions");
...
Reply with only {token}, then stop. Do not read other notes. No bash."
    ...
    async fn live_low_precision_does_not_block_normal_work() {
        ...
        let dir =
            std::env::temp_dir().join(format!("hyper-lossy-rer-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(&dir).unwrap();
        std::fs::write(dir.join("src.txt"), "alpha\n").unwrap();
        let (out, tools) = run_lossy_live(
            &cfg,
