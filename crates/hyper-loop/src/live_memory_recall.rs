...
mod tests {
    ...
    const REPORT: &str = "/tmp/hyper-memory-recall.json";

    const SYSTEM: &str = "Workspace assistant. Paths are relative.";
    const SYSTEM_PTR: &str =
        "Workspace assistant. Paths are relative.\nPrefs in MEMORY.md. Do not read it for local code edits.";
...
开发机 SSH 可能是某台内网主机，具体以当时环境为准。";
    ...
    async fn live_memory_recall_naked_notools() {
        run_matrix(false, "/tmp/hyper-memory-recall-notools.json").await;
    }
}
