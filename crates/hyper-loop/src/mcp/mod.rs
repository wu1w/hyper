...
impl McpRegistry {
    ...
    pub fn load(home: Option<&Path>, workspace: &Path, base: &McpConfig) -> Self {
        ...
        merge_file(
            &mut servers,
            &mut timeout_s,
            &workspace.join(".grok-hyper").join("mcp.toml"),
        );
        merge_dir(&mut servers, &workspace.join(".grok-hyper").join("mcp"));
    ...
    write_rpc(
        ...
        json!({
            ...
            "params": {
                ...
                "capabilities": {},
                "clientInfo": {"name": "hyper", "version": "0.1.0"}
            }
        }),
    )
    .await?;
...
"#,
    ...
    fn workspace_mcp_toml_overlays_home() {
        let root =
            std::env::temp_dir().join(format!("hyper-mcp-ov-{}", uuid::Uuid::new_v4().simple()));
        let home = root.join("home");
        let ws = root.join("ws");
        std::fs::create_dir_all(home.join("mcp")).unwrap();
        std::fs::create_dir_all(ws.join(".grok-hyper").join("mcp")).unwrap();
        std::fs::write(
            home.join("mcp.toml"),
        ...
        .unwrap();
        std::fs::write(
            ws.join(".grok-hyper").join("mcp.toml"),
            "[[servers]]\nname=\"docs\"\ncommand=\"python3\"\nargs=[\"ws.py\"]\ndescription=\"ws docs\"\nmethods=[\"search\"]\n",
        )
        .unwrap();
        std::fs::write(
            ws.join(".grok-hyper").join("mcp").join("gh.toml"),
            "name=\"gh\"\ncommand=\"npx\"\nargs=[\"-y\",\"github\"]\n",
        )
        .unwrap();
    ...
    fn overlay_without_timeout_keeps_base_timeout() {
        ...
        let root =
            std::env::temp_dir().join(format!("hyper-mcp-to-{}", uuid::Uuid::new_v4().simple()));
        let home = root.join("home");
        let ws = root.join("ws");
...
"#,
    ...
    async fn timeout_includes_stderr_tail() {
        let dir =
            std::env::temp_dir().join(format!("hyper-mcp-tmo-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(&dir).unwrap();
...
"#,
    ...
    async fn timeout_marks_server_dead_for_the_turn() {
        let dir =
            std::env::temp_dir().join(format!("hyper-mcp-dead-{}", uuid::Uuid::new_v4().simple()));
        std::fs::create_dir_all(&dir).unwrap();
        let py = dir.join("hang_mcp.py");
    ...
    fn host_env_with_dummy_api_key() -> BTreeMap<String, String> {
        ...
        host.insert("HYPER_API_KEY".into(), "dummy-hyper-api-key".into());
        host
    }

    #[test]
    fn apply_mcp_env_omits_host_hyper_api_key_by_default() {
        let spec = McpServer {
            name: "t".into(),
            command: "true".into(),
            ..McpServer::default()
        ...
        assert!(
            !env.contains_key("HYPER_API_KEY"),
        ...
        assert!(!over.contains_key("HYPER_API_KEY"));
        ...
        let leaked = cmd
            .as_std()
            .get_envs()
            .any(|(k, v)| v.is_some() && k.to_string_lossy() == "HYPER_API_KEY");
        assert!(!leaked);
    }

    #[test]
    fn apply_mcp_env_keeps_host_hyper_api_key_when_inherit_env() {
        let spec = McpServer {
            name: "t".into(),
            command: "true".into(),
            inherit_env: true,
