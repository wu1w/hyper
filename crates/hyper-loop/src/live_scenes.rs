...
mod tests {
    ...
    async fn live_office_cutoff_trace() {
        ...
        let path = std::env::temp_dir().join("hyper-office-cutoff-trace.json");
    ...
    async fn live_roleplay_prefix_ablation() {
        ...
        // force "coding agent" identity + tools
        {
            ...
            if let Some(sys) = msgs.iter_mut().find(|m| m.role == "system") {
                if let Some(c) = sys.content.as_mut() {
                    *c = c.replacen(
                        "an agent in this workspace",
                        "a marked agent in this workspace",
                        1,
                    );
                }
            }
        ...
        let path = std::env::temp_dir().join("hyper-roleplay-ablation.json");
        std::fs::write(&path, serde_json::to_string_pretty(&report).unwrap()).unwrap();
        for v in &variants {
            eprintln!(
                "\n===== {} wall={} tools={} =====\n{}\n--- think ---\n{}\n",
    ...
    async fn live_agent_vs_naked_three_scenes() {
        ...
        let out = std::env::temp_dir().join("hyper-scene-compare.json");
