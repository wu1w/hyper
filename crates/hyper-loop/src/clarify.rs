//! Blocking multiple-choice ask. Frozen `agent_tools()` includes `AskQuestion`.
//!
//! `--print` and YOLO skip to the first option. Missing ClarifyHub is an error
//! (do not fake Skip). `/plan` and `/clarify` arm the overlay. Does not replace permit.
