//! Newline-delimited JSON-RPC 2.0 for `hyper --sidecar`.
//!
...
pub(crate) enum EventStore {
    ...
    /// `~/.grok-hyper/sessions` cannot be created.
...
impl SidecarSession {
    ...
    pub fn imagine_mode(&self) -> bool {
        self.imagine_mode
    }

    pub fn set_imagine_mode(&mut self, on: bool) {
    ...
    pub fn state_json(&self) -> serde_json::Value {
        json!({
            ...
            "window": self.window,
            "subagents": crate::subagent::snapshot_json(&self.session_id),
...
mod tests {
    ...
    #[test]
    fn user_edit_records_hidden_note() {
        let mut session = SidecarSession::new(SidecarOpts::default());
        let open = parse_request_line(
            r#"{"jsonrpc":"2.0","id":1,"method":"session.open","params":{"session":"s-edit","workspace":"/tmp/ws","mode":"agent"}}"#,
        )
        .unwrap();
        ...
        let req = parse_request_line(
            r#"{"jsonrpc":"2.0","id":2,"method":"session.user_edit","params":{"path":"out.docx","kind":"word","bytes":8,"sha256":"ab"}}"#,
        )
        .unwrap();
        match session.handle(&req) {
            Dispatch::Result { result, events } => {
                ...
                let SessionEvent::User(u) = &events[0] else {
                    panic!("expected user event");
                };
                assert!(u.text.contains("[user-edited]"), "{}", u.text);
                assert!(u.text.contains("out.docx"), "{}", u.text);
                assert!(crate::template::is_hidden_user_text(&u.text));
            }
            other => panic!("{other:?}"),
        }
    }
    ...
    fn set_window_is_visible_and_frozen_into_turn_snapshot() {
        ...
        assert_eq!(session.window(), crate::config::CODING_CTX_TOKENS);
        assert_eq!(session.snapshot().window, crate::config::CODING_CTX_TOKENS);
        assert_eq!(
            session.state_json()["window"],
            crate::config::CODING_CTX_TOKENS
        );
    }
    ...
    fn cli_locked_think_survives_session_open() {
        ...
        assert_eq!(snap.policy.max_tokens, 0);
    ...
    fn slash_help_is_local() {
        ...
        match session.handle(&req) {
            Dispatch::Result { result, events } => {
                ...
                assert!(text.contains("/imagine"), "{text}");
            }
            other => panic!("{other:?}"),
        }
    }
    ...
    #[test]
    fn slash_imagine_arms_mode_and_prompt_starts_turn() {
        let mut session = open_mem();
        let on = parse_request_line(
            r#"{"jsonrpc":"2.0","id":2,"method":"slash","params":{"text":"/imagine on"}}"#,
        )
        .unwrap();
        match session.handle(&on) {
            Dispatch::Result { result, .. } => {
                assert!(result["text"].as_str().unwrap().contains("Imagine on"));
            }
            other => panic!("{other:?}"),
        }
        assert!(session.imagine_mode());
        assert!(session.snapshot().imagine_mode);
        assert_eq!(session.state_json()["imagine_mode"], true);

        let gen = parse_request_line(
            r#"{"jsonrpc":"2.0","id":3,"method":"slash","params":{"text":"/imagine a red cube"}}"#,
        )
        .unwrap();
        match session.handle(&gen) {
            Dispatch::TurnStart { prompt, .. } => assert_eq!(prompt, "a red cube"),
            other => panic!("{other:?}"),
        }
        assert!(session.imagine_mode());
    }
