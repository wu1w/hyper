...
impl SidecarSession {
    ...
    pub(crate) fn fork_session(&mut self, directive: Option<String>) -> Dispatch {
        if self.turn_in_flight && directive.as_ref().is_none_or(|d| d.trim().is_empty()) {
            return Dispatch::Error(RpcError::internal("turn in progress"));
        }
        ...
        match &mut self.store {
            EventStore::Log(log) => match log.fork(start.clone()) {
                Ok(forked) => {
            ...
            EventStore::Memory(events) => {
                ...
                let mut copied = events.clone();
                copied[0] = SessionEvent::Start(start.clone());
                copied.push(fork.clone());
                self.store = EventStore::Memory(copied);
            }
        }
        ...
        if let Some(prompt) = directive {
            if self.turn_in_flight {
                return self.enqueue_prompt(prompt, false);
            }
            self.turn_in_flight = true;
            return Dispatch::turn(prompt);
        }
        ...
        Dispatch::Result {
            result: json!({
                ...
                "session": new_id,
                "from": from_id,
                "note": "git worktree isolation is not implemented",
            }),
            events: vec![SessionEvent::Start(start), fork],
        }
    }

    pub(crate) fn session_user_edit(&mut self, params: Option<&Value>) -> Dispatch {
        if !self.opened {
            return Dispatch::Error(RpcError::invalid_request("session not open"));
        }
        let p: UserEditParams = match parse_params(params) {
            Ok(p) => p,
            Err(e) => return Dispatch::Error(e),
        };
        let path = p.path.trim().to_string();
        if path.is_empty() {
            return Dispatch::Error(RpcError::invalid_params("path is required"));
        }
        let note = crate::office_edit::format_user_edit_note(&crate::office_edit::UserEdit {
            path: path.clone(),
            kind: p.kind,
            bytes: p.bytes,
            sha256: p.sha256,
        });
        let event = SessionEvent::user(crate::template::wrap_tool_response(&note));
        self.record(event.clone());
        if self.turn_in_flight {
            self.mailbox.push_steer(note);
        }
        Dispatch::Result {
            result: json!({"ok": true, "path": path}),
            events: vec![event],
        }
    }
    ...
    pub(crate) fn bind_store(&mut self, start: SessionStart) -> Vec<SessionEvent> {
        ...
        match SessionLog::open(&self.session_id) {
            ...
            Err(_) => match SessionLog::create(start.clone()) {
                ...
                Err(e) => {
                    eprintln!(
                        "hyper: session JSONL unavailable ({e}); this session stays in memory"
                    );
                    let event = SessionEvent::Start(start);
                    self.store = EventStore::Memory(vec![event.clone()]);
                    vec![event]
...
mod fork_session_tests {
    ...
    fn open_mem() -> SidecarSession {
        ...
        let open = parse_request_line(
            r#"{"jsonrpc":"2.0","id":1,"method":"session.open","params":{"session":"s-fork","workspace":"/tmp/ws","mode":"agent"}}"#,
        )
        .unwrap();
    ...
    #[test]
    fn fork_session_copies_history_without_worktree() {
        let mut session = open_mem();
        session.record(SessionEvent::user("keep me"));
        let old = session.session_id.clone();
        match session.fork_session(None) {
            Dispatch::Result { result, events } => {
                assert_eq!(result["ok"], true);
                assert_ne!(session.session_id, old);
                assert_eq!(result["from"], old);
                assert!(
                    result["note"].as_str().unwrap().contains("worktree"),
                    "{result}"
                );
                ...
                assert!(session
                    .events()
                    .iter()
                    .any(|e| matches!(e, SessionEvent::User(_))));
            }
            other => panic!("{other:?}"),
        }
    }
    ...
    #[test]
    fn fork_session_directive_starts_turn() {
        let mut session = open_mem();
        match session.fork_session(Some("keep going".into())) {
            Dispatch::TurnStart { prompt, .. } => assert_eq!(prompt, "keep going"),
            other => panic!("{other:?}"),
        }
    }
}
