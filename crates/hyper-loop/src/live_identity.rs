...
mod tests {
    ...
    const REPORT: &str = "/tmp/hyper-identity-report.json";
...
只陈述、编号、归档。";
    ...
    fn agent_md_for(role: Role, inj: Inject) -> Option<String> {
        match inj {
            Inject::Tiny | Inject::Memory | Inject::User0 => {
                Some("Workspace assistant. Paths are relative.".into())
            }
    ...
    fn identity_token_table() {
        let rows = [
            ("tiny", "Workspace assistant. Paths are relative."),
            ("tea_line", TEA_LINE),
            ("tea_soul", TEA_SOUL),
            ("tea_fat", TEA_FAT),
            ("arch_line", ARCH_LINE),
