...
use super::verify::{is_code_path, is_test_path};
...
pub const TEST_EXPECTATION_NOTE: &str = "[trajectory] An existing test expectation was edited. \
Current tests may be spec or they may need to move with the request; check the user, docs, \
and implementation evidence, then continue, change course, or explain the conflict.";

/// Injected on a detected revert pair, at most twice per session.
pub const THRASH_NOTE: &str = "[trajectory] The same location was just reverted; the trajectory may be thrashing. \
Summarize the evidence and current direction first; if it is thin, change the observation point, else continue or finish.";
...
pub const TEST_RED_NOTE: &str = "[trajectory] Tests went green to red after a production-only edit; test files were not changed. \
This is new counter-evidence; check implementation, spec, and the failure, then keep fixing, revert, or explain the conflict.";
...
impl EditGuard {
    ...
    pub fn observe(
        ...
        if matches!(crate::tools_schema::dispatch_name(name), "edit" | "write") {
        ...
        if crate::tools_schema::dispatch_name(name) == "write" {
            return self.observe_write(args, prior_write);
        }
        if crate::tools_schema::dispatch_name(name) != "edit" {
            return Vec::new();
        }
...
mod tests {
    ...
    #[test]
    fn office_doc_edit_does_not_want_oracle() {
        let mut g = EditGuard::new();
        obs(&mut g, "edit", edit("notes.md", "a", "b"));
        assert!(!g.wants_oracle());
    }
}
