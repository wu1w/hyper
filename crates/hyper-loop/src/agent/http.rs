...
impl HttpCompleter {
    ...
    async fn post(
        ...
        if let Some(id) = lock_conv(&self.conv_id).clone() {
            if grok_like_url(&self.url) {
...
impl Completer for HttpCompleter {
    ...
    fn pin_session(&self, session_id: &str) {
        ...
        *lock_conv(&self.conv_id) = if id.is_empty() {
            None
        } else {
            Some(id.to_string())
        };
...
fn grok_like_url(url: &str) -> bool {
    ...
    u.contains("api.x.ai") || u.contains("grok.com") || u.contains("cli-chat-proxy")
}
...
mod tests {
    ...
    fn configured_model_skips_models_list() {
        ...
        assert_eq!(
            configured_chat_model(&Config::default()).as_deref(),
            Some("grok-4.6")
        );
    }
