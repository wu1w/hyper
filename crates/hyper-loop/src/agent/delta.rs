...
impl StdioState {
    ...
    pub(super) fn close_think(&self) {
        if self.think_open.swap(false, Ordering::Relaxed) {
            eprintln!();
        }
    }
}
...
impl TokenSink {
    ...
    /// Host-side web/X search (not a client function_call). Stderr tag or a
    /// live tool card; the agent loop must not try to execute these.
    pub(super) fn tool_tag(&self, name: &str, preview: &str) {
        match &self.kind {
            TokenSinkKind::Events(emit) => {
                let id = format!(
                    "server-{name}-{}",
                    preview.chars().take(24).collect::<String>()
                );
                emit.append(SessionEvent::tool(id, name, preview));
            }
            TokenSinkKind::Stdio(state) => {
                if state.think_open.swap(false, Ordering::Relaxed) {
                    eprintln!();
    ...
    fn is_stdio(&self) -> bool {
        matches!(self.kind, TokenSinkKind::Stdio(_))
    }
}
...
impl StreamPaint {
    pub(super) fn new(sink: TokenSink) -> Self {
        let defer_stdio = sink.is_stdio();
        Self {
            sink,
            think_n: 0,
            text_n: 0,
            hide_text: defer_stdio,
            defer_stdio,
            started: false,
        }
    }
    ...
    pub(super) fn finish(&mut self, reasoning: &str, content: &str, has_tools: bool) {
        if self.defer_stdio && !has_tools {
            self.hide_text = false;
            let (_, text) = visible_raw(reasoning, content);
            self.emit_text(&text);
        }
    }

    pub(super) fn tool_tag(&mut self, name: &str, preview: &str) {
        self.start();
        self.sink.tool_tag(name, preview);
    }
...
mod tests {
    ...
    #[test]
    fn stdio_holds_narration_on_tool_hops() {
        let state = Arc::new(StdioState::default());
        let mut paint = StreamPaint::new(TokenSink::stdio(state.clone()));
        paint.push_raw("plan", "I'll update src/lib.rs", false);
        assert!(
            !state.text_streamed(),
            "stdio must not print before the hop ends"
        );
        paint.finish("plan", "I'll update src/lib.rs", true);
        assert!(!state.text_streamed(), "tool hops must stay off stdout");
    }

    #[test]
    fn stdio_prints_final_answer_without_tools() {
        let state = Arc::new(StdioState::default());
        let mut paint = StreamPaint::new(TokenSink::stdio(state.clone()));
        paint.push_raw("", "done", false);
