...
fn web_search_query(args: &Value) -> Option<String> {
...
impl WebRunner {
    ...
    pub async fn run(
        ...
        let query = web_search_query(&call.arguments).unwrap_or_default();
        let url = arg_str(&call.arguments, "url").unwrap_or_default();
        if call.name == "WebFetch" {
            if url.trim().is_empty() {
                return ToolResponse::text(
                    &call.id,
                    "Error: WebFetch needs `url`.",
                    ToolState::Error,
                );
            }
        ...
        if call.name == "WebSearch" {
            if query.is_empty() {
                return ToolResponse::text(
                    &call.id,
                    "Error: WebSearch needs `search_term` or `query`.",
                    ToolState::Error,
                );
            }
            return self.search(call, &query, limits, blobs).await;
        }
        if !url.trim().is_empty() {
            return self.fetch(call, url.trim(), limits, blobs).await;
        }
        ...
        ToolResponse::text(
            &call.id,
            "Error: need query (search) or url (fetch page text).",
            ToolState::Error,
        )
    }
    ...
    async fn search(
        ...
        ToolResponse::text(
            &call.id,
            format!(
                "Error: no search results ({}). Try different keywords.",
                errors.join("; ")
            ),
            ToolState::Error,
        )
    ...
    async fn tavily_search(&self, query: &str, n: usize) -> Result<Vec<Hit>, String> {
        let key = self
            .tavily_key
            .as_deref()
            .ok_or("Tavily key is not set (config.toml [web] tavily_api_key or TAVILY_API_KEY)")?;
        let body = json!({
            "api_key": key,
            "query": query,
            "max_results": n,
...
fn format_hits(_source: &str, query: &str, hits: &[Hit]) -> String {
    let mut s = format!("[web] {} result(s): {query}\n", hits.len());
...
<footer>版权所有</footer></body></html>"#;
    ...
    fn hit_format_is_uniform() {
        ...
        assert_eq!(s, "[web] 1 result(s): q\n1. T\n   https://u\n   S\n");
    }

    #[test]
    fn web_search_query_skips_empty_search_term() {
        assert_eq!(
            web_search_query(&json!({"query": "rust async"})).as_deref(),
            Some("rust async")
        );
        assert_eq!(
            web_search_query(&json!({"search_term": "", "query": "from alias"})).as_deref(),
            Some("from alias")
        );
        assert_eq!(
            web_search_query(&json!({"search_term": "  ", "query": "from alias"})).as_deref(),
        ...
        assert_eq!(
            web_search_query(&json!({"search_term": "preferred", "query": "ignored"})).as_deref(),
            Some("preferred")
        );
        assert!(web_search_query(&json!({"search_term": "", "query": "  "})).is_none());
        assert!(web_search_query(&json!({})).is_none());
    }

    #[tokio::test]
    async fn websearch_errors_when_both_aliases_empty() {
        let r = WebRunner::new(WebConfig::default(), &McpRegistry::default());
        let call = ToolCall {
            id: "t1".into(),
            name: "WebSearch".into(),
            arguments: json!({"search_term": "", "query": "  "}),
        };
        let resp = r.run(&call, ToolLimits::default(), None).await;
