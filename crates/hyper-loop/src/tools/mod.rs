...
/// Foreign `{stem}.hypertmp.*` leftovers older than this are removed on the next
...
pub(super) fn cleanup_stale_tmp_older_than(dir: &Path, stem: &str, min_age: Duration) {
    let prefix = format!("{stem}.hypertmp.");
...
pub async fn run_tool(
    ...
    match crate::tools_schema::dispatch_name(&call.name) {
        ...
        "task" | "awaitshell" => {
            ...
            crate::subagent::dispatch(
                &owned,
                &crate::subagent::DispatchCtx::from_workspace(workspace.root().to_path_buf()),
            )
            .await
        }
...
mod tests {
    ...
    #[test]
    fn cursor_aliases_write_glob_grep_delete_todo() {
        let (ws, dir) = scratch();
        let w = fs::write_file(
            &ws,
            &call(
                "Write",
                json!({"path": "notes.md", "contents": "hello office\n"}),
            ),
        );
        assert_eq!(w.state, ToolState::Success);
        let g = find::glob_files(
            &ws,
            &call("Glob", json!({"glob_pattern": "**/*.md"})),
            ToolLimits::default(),
        );
        assert!(g.joined_text().contains("notes.md"), "{}", g.joined_text());
        let hit = find::grep_files(
            &ws,
            &call("Grep", json!({"pattern": "office", "glob": "*.md"})),
            ToolLimits::default(),
            None,
        );
        assert!(
            hit.joined_text().contains("notes.md"),
            "{}",
            hit.joined_text()
        );
        let todos = todo::todo_write(
            &ws,
            &call(
                "TodoWrite",
                json!({
                    "merge": false,
                    "todos": [{"id": "1", "content": "edit the note", "status": "in_progress"}]
                }),
            ),
        );
        assert_eq!(todos.state, ToolState::Success);
        assert!(todos.joined_text().contains("edit the note"));
        let d = fs::delete_file(&ws, &call("Delete", json!({"path": "notes.md"})));
        assert_eq!(d.state, ToolState::Success);
        assert!(!dir.join("notes.md").exists());
        let _ = std::fs::remove_dir_all(dir);
    }

    #[tokio::test]
    async fn run_tool_accepts_cursor_and_legacy_names() {
        let _g = crate::subagent::lock_registry_for_test().await;
        let (ws, dir) = scratch();
        let cancel = CancelFlag::new();
        let w = run_tool(
            &ws,
            &call("Write", json!({"path": "a.txt", "contents": "x\n"})),
            cancel.clone(),
            ToolLimits::default(),
            false,
            None,
        ...
        .await;
        assert!(r.joined_text().contains("x"), "{}", r.joined_text());
        let t = run_tool(
            &ws,
            &call("Task", json!({"prompt": "explore the notes"})),
            cancel,
            ToolLimits::default(),
            false,
            None,
