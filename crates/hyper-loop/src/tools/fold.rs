...
/// Content-addressed tool-result archive (`~/.grok-hyper/blobs/{sha256}`).
#[derive(Clone, Debug)]
...
/// `{stem}.hypertmp.{pid}.{nanos}.{uuid}` next to the blob. Two writers cannot
/// share `{sha}.hypertmp` the way `path.with_extension("hypertmp")` did.
...
mod tests {
    ...
    fn fold_high_result_max_still_folds_20k() {
        let dir =
            std::env::temp_dir().join(format!("hyper-blobs-{}", uuid::Uuid::new_v4().simple()));
    ...
    fn put_leaves_no_tmp_and_cleans_foreign_pid() {
        ...
        let stale = dir.join(format!("{sha}.hypertmp.{other}.0.deadbeef"));
    ...
    fn tmp_guard_removes_our_tmp_on_drop() {
        let dir =
            std::env::temp_dir().join(format!("hyper-blobs-{}", uuid::Uuid::new_v4().simple()));
        fs::create_dir_all(&dir).unwrap();
        let tmp = dir.join("x.hypertmp.1.0.gone");
