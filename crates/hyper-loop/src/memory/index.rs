...
mod tests {
    ...
    fn tmp() -> std::path::PathBuf {
        let dir =
            std::env::temp_dir().join(format!("hyper-mem-idx-{}", uuid::Uuid::new_v4().simple()));
    ...
    fn hyphenated_and_cjk_query_hits() {
        ...
        idx.upsert(
            ...
            "daily",
            "compact note: grok-hyper audit 源码 and related work",
        )
        .unwrap();
        let hits = idx.search("grok-hyper audit 源码修改", 10).unwrap();
    ...
    fn hyphen_query_does_not_glue_tokens() {
        let dir = tmp();
        let idx = MemoryIndex::open(&dir).unwrap();
        idx.upsert("notes/repo.md", "note", "clone grok-hyper then probe")
            .unwrap();
        let q = fts_query("grok-hyper");
        assert!(!q.contains("qharness"), "{q}");
        assert!(q.contains("\"grok\""), "{q}");
        assert!(q.contains("\"hyper\""), "{q}");
        let hits = idx.search("grok-hyper", 10).unwrap();
