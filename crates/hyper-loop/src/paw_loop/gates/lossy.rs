...
pub const NAME_NOTE: &str = "[trajectory] The same tool and arguments have landed several times. \
If the last result added nothing, change arguments or tools, or finish from the evidence you have.";

/// One-shot observation when the same path keeps being touched the same way.
pub const PATH_NOTE: &str =
    "[trajectory] The same operation on the same path has landed several times. \
Change offset or target, or continue from what you already read.";
