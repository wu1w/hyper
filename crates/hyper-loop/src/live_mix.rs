...
//!
//! Gated (`cargo test -- --ignored`). `HYPER_MIX_ROUNDS` overrides length
...
If it errors, read scripts/emit.py and fix. Then stop. No cargo test."
    ...
    fn count_rs_lines(root: &Path) -> usize {
        ...
        while let Some(dir) = stack.pop() {
            ...
            for e in rd.flatten() {
                ...
                if name == "target" || name == ".git" || name == ".hyper-home" {
                    continue;
                }
                if p.is_dir() {
                    stack.push(p);
    ...
    async fn live_mix_300_chat_docs_code_image_skill_mcp() {
        ...
        let cfg = live_cfg();
        let dir =
            std::env::temp_dir().join(format!("hyper-live-mix-{}", uuid::Uuid::new_v4().simple()));
