...
use hyper_loop::clarify::{ClarifyDecision, ClarifyRequest};
use hyper_loop::config::Config;
use hyper_loop::permit::{PermitDecision, PermitRequest};
...
fn save_setup(_cfg: &Config, wiz: &SetupWizard) -> Result<(Config, String), String> {
    ...
    if std::env::var("HYPER_BASE_URL")
        ...
        || std::env::var("HYPER_MODEL")
            ...
            .is_some()
    {
        note.push_str("  (HYPER_* env still wins on next TUI/CLI start; hyper web uses the file)");
    }
    Ok((next, note))
}
