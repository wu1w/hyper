...
use hyper_loop::clarify::ClarifyHub;
use hyper_loop::config::Config;
use hyper_loop::permit::{ApprovalMode, PermitHub};
use hyper_loop::policy::ThinkPolicy;
use hyper_loop::session::{catalog, new_session_id, SessionEvent, SessionLog, SessionMode};
use hyper_loop::sidecar::{
    Dispatch, EventSink, PolicyCaps, RpcRequest, SidecarOpts, SidecarSession, TurnRequest,
};
use hyper_loop::{CancelFlag, MediaPart};
...
struct LiveTurn {
    join: JoinHandle<hyper_loop::sidecar::TurnResult>,
    cancel: CancelFlag,
}

pub async fn run(opts: TuiOpts) -> Result<ExitCode> {
    if !std::io::stdin().is_terminal() || !stdout().is_terminal() {
        anyhow::bail!("hyper TUI needs a TTY (use --print for oneshot)");
    }
    ...
    transcript.push_system(format!(
        "hyper  {}  {}  /setup  /plan  /approvals  /help",
        snap.model,
        snap.mode.as_str()
    ));
        ...
        tokio::select! {
            ...
            result = await_turn(&mut live) => {
                if let Some(result) = result {
                    ...
                    if let Some((next, parts)) = session.pop_follow_up() {
                        start_turn(
                            &mut session,
                            &mut transcript,
                            &mut live,
...
async fn await_turn(live: &mut Option<LiveTurn>) -> Option<hyper_loop::sidecar::TurnResult> {
    if live.is_none() {
    ...
    *live = None;
    match result {
        ...
        Err(_) => Some(hyper_loop::sidecar::TurnResult::fail("turn task failed")),
    }
}
...
fn draw(
    ...
    let imagine = if snap.imagine_mode { " · imagine" } else { "" };
    let info = format!(
        " {plan} · {} · {} · {}{}{clarify}{imagine} · /setup /plan /imagine /clarify /approvals /lossy ",
