...
use hyper_loop::policy::{Effort, ThinkPolicy, XHIGH_WARN};
use hyper_loop::probe::{run_probe, write_report};
use hyper_loop::session::{catalog, SessionLog};
...
struct Cli {
    ...
    /// Thinking effort: low | medium | high | xhigh. Bare `--think` means medium.
    #[arg(
        long,
        value_name = "LEVEL",
        num_args = 0..=1,
...
enum Command {
    ...
    /// Install hyper as a dsh profile (optional stdio client; not the product shell).
    #[command(name = "dsh-install")]
    DshInstall {
        /// dsh profile name. Default `hyper`.
    ...
    /// OnlyOffice Document Server (Docker). `hyper web` starts it automatically.
    Office {
        #[command(subcommand)]
        action: office::OfficeAction,
    },
    /// Sign in with SpaceXAI OAuth (`grok login --oauth`). Writes ~/.grok/auth.json.
    Login {
        /// Headless device-code flow (`grok login --device-auth`).
        #[arg(long)]
        device_auth: bool,
    },
    /// Clear the grok login session (`grok logout`).
    Logout,
}
...
async fn real_main() -> Result<ExitCode> {
    ...
    match cli.command {
        ...
        Some(Command::Web {
            ...
            // Empty = hyper-web uses [console] workspace from config.toml, else cwd.
            let workspace = cli.workspace.clone().unwrap_or_default();
            hyper_web::run(hyper_web::WebOpts {
                bind,
                allow_lan,
                workspace,
                session_id: cli.session.clone().unwrap_or_default(),
                open_browser: !no_open,
                agents_md: !cli.no_agents_md,
                agents_md_head: cli.agents_md_head,
            })
            .await
        }
        Some(Command::Office { action }) => office::run(action).await,
        Some(Command::Login { device_auth }) => login_cmd(device_auth).await,
        Some(Command::Logout) => {
            hyper_loop::auth::logout_session().context("logout")?;
            eprintln!("logged out");
            Ok(ExitCode::SUCCESS)
        }
        None => {
            ...
            if prompt.trim().is_empty() && !cli.print {
                if std::io::stdin().is_terminal() && std::io::stdout().is_terminal() {
                    return hyper_tui::run(hyper_tui::TuiOpts {
                        cfg,
                        workspace,
                        session_id,
                        mode,
            ...
            opts.agents_md = !cli.no_agents_md;
            ...
            let completer = tokio::select! {
                ...
                c = TransportCompleter::connect(&cfg, policy) => c.context("connect")?,
            };
            eprintln!("model: {}", completer.model());
            eprintln!("wire: {}", completer.wire());
            if let Some(effort) = completer.effort_label() {
...
fn intercept_slash(
    ...
    let persist = match &cmd {
        ...
        | SlashCmd::Approvals { .. }
        | SlashCmd::Clarify { .. }
        | SlashCmd::Imagine { prompt: None, .. }
        | SlashCmd::LowPrecision { .. } => false,
        SlashCmd::Plan {
            prompt: Some(p), ..
        } if !p.trim().is_empty() => true,
        SlashCmd::Plan { .. } => false,
        SlashCmd::Imagine {
            prompt: Some(p), ..
        } if !p.trim().is_empty() => true,
...
fn think_policy(cfg: &Config, fast: bool, think: Option<&str>, mode: Option<&str>) -> ThinkPolicy {
    let p = ThinkPolicy::from_cli(&cfg.think_budget(), fast, think, mode);
...
fn print_human(report: &hyper_loop::ProbeReport, path: &std::path::Path) {
    println!("grok-hyper probe");
...
async fn login_cmd(device_auth: bool) -> Result<ExitCode> {
    use hyper_loop::oauth::{
        authorize_url, generate_pkce, open_system_browser, persist_oauth_tokens, poll_device_until,
        random_state, request_device, wait_callback,
    };
    ...
    if device_auth {
        ...
        let tokens = poll_device_until(&device, &CancelFlag::new())
            .await
            .context("device poll")?;
        persist_oauth_tokens(tokens).context("write session")?;
        eprintln!("logged in (device code)");
        return Ok(ExitCode::SUCCESS);
    }
    ...
    let url = authorize_url(&pkce, &state, &nonce);
    eprintln!("Open this URL to sign in:\n{url}");
    open_system_browser(&url);
    let code = wait_callback(&state, &CancelFlag::new())
        .await
        .context("oauth callback")?;
    let tokens = hyper_loop::oauth::exchange_code(&code, &pkce.verifier)
        .await
        .context("oauth token")?;
    persist_oauth_tokens(tokens).context("write session")?;
    eprintln!("logged in (OAuth)");
    Ok(ExitCode::SUCCESS)
}
...
mod clap_tests {
    ...
    fn dsh_install_is_not_swallowed_as_a_prompt() {
        let cli = Cli::try_parse_from(["hyper", "dsh-install", "--dry-run"]).expect("parse");
        match cli.command {
            Some(Command::DshInstall {
                dry_run,
                profile,
    ...
    fn web_is_not_swallowed_as_a_prompt() {
        let cli = Cli::try_parse_from(["hyper", "web", "--no-open", "--bind", "127.0.0.1:0"])
            .expect("parse");
        match cli.command {
            Some(Command::Web {
                bind,
    ...
    #[test]
    fn login_is_not_swallowed_as_a_prompt() {
        let cli = Cli::try_parse_from(["hyper", "login", "--device-auth"]).expect("parse");
        match cli.command {
