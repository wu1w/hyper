...
use hyper_loop::tokenize::count_tokens;
use hyper_loop::Family;

pub const EXPECTED_BANNER: &str = "hyper-ok";
...
The file must contain println!(\"new\") and must not contain old. Change only what is required.",
    ...
    Task {
        ...
        prompt: "Read hint.txt. Change the BANNER constant in src/lib.rs so the program would print hyper-ok. \
...
pub fn validate_fixtures() -> Result<Vec<String>> {
    let tmp = std::env::temp_dir().join(format!("hyper-bench-validate-{}", std::process::id()));
