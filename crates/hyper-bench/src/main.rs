//! Three-task quality gate for grok-hyper.
//!
//! Default: validate fixtures and print the checklist (no model).
//! `--live`: run each task with hyper's default adaptive thinking policy (or the
//! hyper-loop Agent if the `hyper` binary is missing).
//!
...
use hyper_loop::policy::ThinkPolicy;
use hyper_loop::{
...
Wall-clock calibration (design Targets; numbers from ~/.grok-hyper/probe.json when present):
  overhead_s      = 2.5
  ttft_cold_gate  = measured_prefix / measured_prefill + overhead_s
  ttft_hot_gate   = typical_suffix / measured_prefill + overhead_s
  wall_4step_off  = 4 * (hot + 200 / decode) + cold
...
#[command(
    name = "hyper-bench",
    about = "Three-task quality gate for grok-hyper (family / profile / quant_label)",
...
fn print_offline(lines: &[String], probe: Option<&ProbeReport>) {
    println!("hyper-bench quality gate (offline)");
    println!("Fixtures:");
    for line in lines {
        println!("  {line}");
    }
...
fn print_calibration(probe: Option<&ProbeReport>) {
    ...
    match probe {
        ...
        None => match Config::probe_path() {
            Ok(p) => println!("  no probe.json at {} — run `hyper probe`", p.display()),
...
fn report_path() -> PathBuf {
    if let Ok(dir) = std::env::var("CARGO_TARGET_DIR") {
        ...
        if p.is_dir() || fs::create_dir_all(&p).is_ok() {
            return p.join("hyper-bench-report.json");
        }
    }
    let ws = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../../target");
    if ws.is_dir() || fs::create_dir_all(&ws).is_ok() {
        return ws.join("hyper-bench-report.json");
    }
    PathBuf::from("hyper-bench-report.json")
}
...
async fn run_live(
    ...
    let (cfg, _) = Config::load_or_init().context("load ~/.grok-hyper/config.toml")?;
    let stamp = unix_stamp();
    let mut tasks = Vec::new();
    let mut model = String::new();

    for task in TASKS {
        let dest = parent.join(format!("hyper-bench-{stamp}-{}", task.name));
        copy_task(task, &dest)?;
...
async fn run_one_task(
    ...
    match find_hyper_bin() {
        Some(bin) => run_via_hyper(&bin, cfg, task, workspace).await,
        None => run_via_agent(cfg, task, workspace, false).await,
    }
}

fn find_hyper_bin() -> Option<PathBuf> {
    if let Ok(p) = std::env::var("HYPER_BIN") {
    ...
    for profile in ["debug", "release"] {
        let cand = target.join(profile).join("hyper");
        if cand.is_file() {
            return Some(cand);
        }
    }
    ...
    let (steps, stop_reason, spawn_notes) = match timed {
        Ok(Ok(out)) => {
            ...
            let notes = if status.success() {
                format!("runner=hyper-bin {} model={model}", bin.display())
            } else {
                format!("runner=hyper-bin exit={status} model={model}")
            };
            (steps, reason, notes)
        }
        Ok(Err(e)) => {
            return Ok(eval_after(
                ...
                format!("runner=hyper-bin error={e}"),
            ));
        }
        Err(_) => {
            return Ok(eval_after(
                ...
                0,
                Some("timeout".into()),
                "runner=hyper-bin timed out".into(),
            ));
        }
    };
    Ok(eval_after(
...
async fn run_via_agent(
    ...
    let policy = ThinkPolicy::native_with(&cfg.think_budget());
...
async fn run_fat_contrast(
    ...
    let lean_dir = parent.join(format!("hyper-bench-{stamp}-fat-lean"));
    copy_task(task, &lean_dir)?;
    let lean = run_via_agent(cfg, task, &lean_dir, false).await?;

    let fat_dir = parent.join(format!("hyper-bench-{stamp}-fat-dummy"));
