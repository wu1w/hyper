...
cargo build --release -p hyper-cli
...
if [ "${HYPER_SIDECAR_SKIP_WIN:-}" = "1" ]; then
  echo "skip Windows hyper.exe (HYPER_SIDECAR_SKIP_WIN=1)" >&2
  exit 0
fi
...
if command -v cargo-xwin >/dev/null 2>&1 || cargo xwin --help >/dev/null 2>&1; then
  ...
  cargo xwin build --release -p hyper-cli --target x86_64-pc-windows-msvc
elif command -v x86_64-w64-mingw32-gcc >/dev/null 2>&1; then
  rustup target add x86_64-pc-windows-gnu >/dev/null
  cargo build --release -p hyper-cli --target x86_64-pc-windows-gnu
else
  echo "skip Windows hyper.exe (install cargo-xwin or mingw-w64 to cross-compile)" >&2
fi
