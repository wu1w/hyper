...
Optional stdio client. **The product shell is `hyper web`**, not dsh.

dsh 只当可选壳。官方 `agent-loop` 关掉，同一套 `hyper --sidecar` 跑工具、ThinkPolicy、JSONL。不要把这个插件当交付面。
...
```
./scripts/install-dsh-hyper.sh
```

会把插件拷到 `~/.grok-hyper/dsh-plugin-hyper` 并注册 dsh profile。日常请用：

```
hyper web
```

模型、key、workspace 仍走 `~/.grok-hyper/config.toml`。dsh 自己的模型页不会驱动 loop。
...
dsh Web / Host
  ctx.agents.followup / cancel
        │
        ▼
dsh-plugin-hyper   (Cordis: setFactory, 翻译 session 事件, 皮肤)
        │  stdio JSON-RPC
        ▼
hyper --sidecar    (Rust: tools / ThinkPolicy / JSONL / compact)
```

`cordis.patch.yml` 把 `agent-loop` 设为 `disabled: true`，插入 `hyper-loop`。dsh 的 fs/bash 工具动物园还在进程里，但 factory 不会调用它们。

## Spawn

```
hyper --sidecar --workspace <dir> --session <id>
```

stdio: `['pipe', 'pipe', 'inherit']`。二进制默认 `hyper`，可用 `HYPER_BIN` 或 patch `config.command` 指到绝对路径。`hyper dsh-install` 会把当前可执行文件写进 profile overlay。
...
| id | 方案 |
|---|---|
| `hyper-ink` | 深色暖炭底 + 青绿强调（默认） |
| `hyper-paper` | 浅色纸色 + 同一青绿 |
...
插件把这些投影成 dsh 的 `user/message` / `assistant/chunk` / `assistant/message` / `tool/call` / `tool/result`，Web 对话区才能画。hyper JSONL 不改。
