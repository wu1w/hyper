/**
 * Cordis Agent factory. dsh UI talks to this; tools/ThinkPolicy stay in hyper.
 */
...
export class Q38Agent {
  ...
  constructor(opts: {
    ...
    this.options = {
      cwd: opts.workspace,
      provider: 'hyper',
      model: opts.model ?? 'hyper',
      ...(opts.agentOptions ?? {}),
    };
    this.projector.model = String(this.options.model ?? 'hyper');
    this.projector.provider = 'hyper';
  }
  ...
  inject(_message: unknown): void {
    // hyper injects skills as hidden user after the live query inside the Rust loop.
  }
  ...
  private async ensureOpen(): Promise<Q38Sidecar> {
    ...
    await this.opening;
    if (!this.sidecar) {
      throw new Error('hyper sidecar failed to start');
    }
    return this.sidecar;
  }
  ...
  private commit(op: DshAppend): void {
    try {
      ...
      process.stderr.write(`hyper-plugin: session.append ${op.type} failed: ${message}\n`);
    }
  }
  ...
  private async run(message: unknown): Promise<void> {
    ...
    try {
      ...
      process.stderr.write(`hyper-plugin: turn failed: ${messageText}\n`);
    }
  }
}
...
export class Q38Factory {
  ...
  private command(): string {
    return this.config.command || process.env.HYPER_BIN || 'hyper';
  }
  ...
  private spawn(ownerCtx: DshCtx, options: Record<string, unknown>, resume: boolean) {
    ...
    const id = sessionIdOf(options, `hyper-${Date.now()}`);
