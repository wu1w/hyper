...
import { injectBootSkin, HYPER_INK } from './skin.js';
...
describe('skin', () => {
  it('injects a style tag once', () => {
    ...
    assert.match(once, /id="hyper-skin"/);
    assert.match(once, new RegExp(HYPER_INK['--dsw-alias-brand-primary']!.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')));
    const twice = injectBootSkin(once);
    assert.equal(twice.split('id="hyper-skin"').length, 2);
  });
});
