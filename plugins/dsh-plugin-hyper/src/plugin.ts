/**
 * Cordis entry: replace dsh agent-loop with hyper sidecar, paint hyper skin.
...
export function apply(ctx: Ctx, config: PluginConfig = {}): void {
  ...
  ctx.effect(() => ctx.agents.setFactory(factory), 'hyper-loop.setFactory()');
