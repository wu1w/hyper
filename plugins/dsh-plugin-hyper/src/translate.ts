/**
 * Map hyper sidecar SessionEvent → dsh session.append ops.
 * dsh UI only understands user/message, assistant/chunk|message, tool/call|result.
 * hyper JSONL stays the source of truth; this is a projection for the shell.
 */
...
export function newId(): string {
  return globalThis.crypto?.randomUUID?.() ?? `hyper-${Date.now()}-${Math.random().toString(16).slice(2)}`;
}
